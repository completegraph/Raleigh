---
title: "Model Building for Crime Prediction 2018-12-31"
author: "Alexander Ng"
...
For the training dataset, the response period measures the crime rates for Jan 1, 2019 - Dec 31, 2019.
We use the `index date` 2018-12-31 to label this script.
...
```{r cubist-varimp}
( ggplot(cubistTune) + labs(title="Cubist Model Tuning", subtitle = "Dec 31, 2019 Base Date") -> p_tuning_cubist )

( ggplot(varImp(cubistTune), top = 40) + labs(title = "Cubist Model Variable Importance", subtitle = "Dec 31, 2019 Base Date") -> p_varimp_cubist )

fname1 = paste0(data_dir, "EXP_CUBIST_MODEL_TUNING_2019-12-31.png")
fname2 = paste0(data_dir, "EXP_CUBIST_MODEL_VARIMP_2019-12-31.png")
...
coef_piv %>% 
  ...
  labs(title = "Distribution of Coefficients of Predictors", 
       subtitle = "from Cubist Committee/Rules  Dec 31, 2019 Base Date",
       x = "Predictors" ,
       y = "Coefficient in Linear Model"
       ) -> p_cubist_coefs_boxplots


cubist_coefs_boxplot_file = paste0(data_dir, "EXP_CUBIST_COEF_BOXPLOT_2019-12-31.png")
...
```{r cubist-predict-2019}
pred_crime_2019 = predict( cubistTune, newdata = X_vars_on_grid_test)  # Base date Dec 31, 2019

(perf_cu = data.frame( modelName = cubistTune$method ,
                    baseDate  = "2019-12-31" ,
                     trainRMSE = getTrainPerf(cubistTune)[1,1] ,
                     trainR2   = getTrainPerf(cubistTune)[1,2] ,
                     testRMSE  = caret::RMSE( pred_crime_2019,  Y_on_grid_test) ,
                     testR2    = caret::R2(   pred_crime_2019,  Y_on_grid_test) 
                     ) ) 


```

```{r cubist-2018-2019-comparison}

pred_crime_2018 = predict( cubistTune, newdata = X_vars_on_grid_train )

predobs_df = tibble( id_grid = all_vars_on_grid_train$id_grid ,
                     train_predicted   = pred_crime_2018 ,
                     train_observed = all_vars_on_grid_train$crime_freq ,
                     test_predicted    = pred_crime_2019 ,
                     test_observed  = all_vars_on_grid_test$crime_freq
                     )
```

```{r cubist-2019-2020-predobs-plot}
predobs_df %>% ggplot(aes(x=train_predicted, y = train_observed)) + 
  ...
  labs( y = "Observed Counts/Cell Year", x = "Predicted Counts/Cell Year", 
        title = "Cubist Model Training 2015-2019", 
...
predobs_plot_file = paste0(data_dir, "CUBIST_PREDOBS_PLOT_2020", ".png" )
...
all_data_on_grid_test %>% mutate( pred_crime_freq = pred_crime_2019 ) -> PAI_cu_source_grid
...
PEI_cu_grid %>% filter( prop_cum_area_best < 0.01 ) %>% ggplot() + geom_line( aes(x = prop_cum_area_best, y = PEI ) ) + 
  labs(title = "PEI of Cubist Model", subtitle = "Base Date Dec 31, 2019")

PAI_cu_ranked_grid %>% filter(prop_cum_area < 0.01 ) %>% ggplot() + geom_line(aes(x = prop_cum_area , y = RRI )) +
  labs(title = "RRI of Cubist Model", subtitle = "Base Date Dec 31, 2019")
...
PAI_cubist_ranked_grid_file = paste0(data_dir, "EXP_CUBIST_PAI_RANKED_GRID_2019-12-31.geojson")

st_write(PAI_cu_ranked_grid ,  dsn = PAI_cubist_ranked_grid_file , delete_dsn = TRUE, quiet = TRUE)

PEI_cubist_csv_grid_file = paste(data_dir, "EXP_CUBIST_PEI_RANKED_CSV_2019-12-31.csv")
...
(PAI_cu_ranked_grid %>% filter( rank <=100 ) %>% ggplot() +
  ...
  geom_sf( data = gj_raleigh_buffer, fill = "black" , alpha = 0.7) +
    ...
    labs(title = paste0("Top 100 Predicted Hotspots by Cubist 2020"), subtitle = paste0("Resolution ", grid_cellsize ," feet" ) ) +
...
hotspot_cubist_map_file = paste0(data_dir, "EXP_CUBIST_HOTSPOT_2020_", grid_cellsize, ".png" )
...
( ggplot(randomForestTune) + labs(title = "Random Forest Tuning", subtitle = "Base Date Dec 31, 2019") -> p_tuning_randomforest )
```

```{r}
( ggplot(varImp(randomForestTune) ) + labs(title = "Random Forest Variable Importance", subtitle = "Dec 31, 2019 Base Date") -> p_varimp_randomforest )

```

```{r}
fname1 = paste0(data_dir, "EXP_RANDOMFOREST_MODEL_TUNING_2019-12-31.png")
fname2 = paste0(data_dir, "EXP_RANDOMFOREST_MODEL_VARIMP_2019-12-31.png")
...
```{r randomforest-predict-2019}
pred_crime_rf_2019 = predict( randomForestTune, newdata = X_vars_on_grid_test)
...
(perf_rf = data.frame( modelName = randomForestTune$method ,
                     ...
                     testRMSE  = caret::RMSE( pred_crime_rf_2019,  Y_on_grid_test) ,
                     testR2    = caret::R2(   pred_crime_rf_2019,  Y_on_grid_test) 
                     ) ) 

```

```{r randomforest-2018-2019-comparison}

pred_crime_rf_2018 = predict( randomForestTune, newdata = X_vars_on_grid_train )

predobs_rf_df = tibble( id_grid        = all_vars_on_grid_train$id_grid ,
                     train_predicted   = pred_crime_rf_2018 ,
                     train_observed    = all_vars_on_grid_train$crime_freq ,
                     test_predicted    = pred_crime_rf_2019 ,
                     test_observed     = all_vars_on_grid_test$crime_freq
                     )
```

```{r randomforest-2019-2020-predobs-plot}
predobs_rf_df %>% ggplot(aes(x=train_predicted, y = train_observed)) + 
  geom_point(color = "red", size = 0.5) + 
  geom_abline(slope=1,intercept=0, color = "grey") + 
  theme(aspect.ratio = 1) +
  labs( y = "Observed Counts/Cell Year", x = "Predicted Counts/Cell Year", 
        title = "Random Forest Training 2016-2019", 
...
predobs_rf_df %>% ggplot(aes(x=test_predicted, y = test_observed)) + 
  ...
  theme(aspect.ratio = 1) +
  labs( y = "Observed Counts/Cell Year", x = "Predicted Counts/Cell Year", 
        title = "Random Forest Model Test 2020", 
...
predobs_rf_plot_file = paste0(data_dir, "RANDOM_PREDOBS_PLOT_2020", ".png" )
...
all_data_on_grid_test %>% mutate( pred_crime_freq = pred_crime_rf_2019 ) -> PAI_rf_source_grid
...
PEI_rf_grid %>% filter( prop_cum_area_best < 0.01 ) %>% ggplot() + geom_line( aes(x = prop_cum_area_best, y = PEI ) ) + 
  labs(title = "PEI of Random Forest Model", subtitle = "Base Date Dec 31, 2019")


PAI_rf_ranked_grid %>% filter(prop_cum_area < 0.01 ) %>% ggplot() + geom_line(aes(x = prop_cum_area , y = RRI )) +
  labs(title = "RRI of Random Forest Model", subtitle = "Base Date Dec 31, 2019")

```

```{r save-randomforest-PAI-ranked-grid}

PAI_rf_ranked_grid_file = paste0(data_dir, "EXP_RANDOMFOREST_PAI_RANKED_GRID_2019-12-31.geojson")

st_write(PAI_rf_ranked_grid ,  dsn = PAI_rf_ranked_grid_file , delete_dsn = TRUE, quiet = TRUE)


PEI_rf_csv_grid_file = paste(data_dir, "EXP_RANDOMFOREST_PEI_RANKED_CSV_2019-12-31.csv")
...
PAI_rf_ranked_grid %>% filter( prop_cum_area < 0.03 ) %>% ggplot() + geom_line(aes( x = rank, y = PAI)) + labs(title = "Random Forest PAI - 2020")
PAI_rf_ranked_grid  %>% ggplot() + geom_line(aes( x = prop_cum_area, y = prop_cum_crime_freq)) + theme(aspect.ratio = 1) + geom_abline(slope = 1, intercept = 0) + labs(title = "AUC Random Forest 2020")
...
(PAI_rf_ranked_grid %>% filter( rank <=100 ) %>% ggplot() +
  ...
  geom_sf( data = gj_raleigh_buffer, fill = "black" , alpha = 0.7) +
    geom_sf( aes(fill = hotspot , color = "red" ) ,  lwd = 0.5  ) +
    labs(title = paste0("Top 100 Predicted Hotspots by Random Forest 2020"), subtitle = paste0("Resolution ", grid_cellsize ," feet" ) ) +
  theme(axis.text = element_blank(), axis.ticks = element_blank(), axis.line = element_blank(), axis.title = element_blank(), 
        legend.position = "none") -> p_hotspot_randomforest_map )

hotspot_randomforest_map_file = paste0(data_dir, "EXP_RANDOMFOREST_HOTSPOT_2020_", grid_cellsize, ".png" )
...
PEI_rf_grid %>% filter( prop_cum_area_best < 0.01 ) %>% ggplot() + geom_line( aes(x = prop_cum_area_best, y = PEI , col = "Random Forest") ) +  geom_line(data = (PEI_cu_grid %>% filter( prop_cum_area_best < 0.01 ) ), aes( x= prop_cum_area_best , y = PEI , col = "Cubist") ) +
  ...
  labs(title = "Predictive Efficiency Index of Models", subtitle = "Base Date Dec 31, 2019", col = "Models", x = "Proportion of Cumulative Area") -> p_PEI
...
PAI_rf_ranked_grid %>% filter(prop_cum_area < 0.01 ) %>% ggplot() + geom_line(aes(x = prop_cum_area , y = RRI , col = "Random Forest")) +
  ...
  scale_color_manual(values = c("Random Forest"= "blue", "Cubist"= "red")) +
  labs(title = "Recapture Rate Index of Models", subtitle = "Base Date Dec 31, 2019", col = "Models", x = "Proportion of Cumulative Area") -> p_RRI
...
PAI_rf_ranked_grid %>% filter(prop_cum_area < 0.01 ) %>% ggplot() + geom_line(aes(x = rank , y = PAI , col = "Random Forest")) +
  ...
  scale_color_manual(values = c("Random Forest"= "blue", "Cubist"= "red")) +
  labs(title = "Predictive Accuracy Index of Models", subtitle = "Base Date Dec 31, 2019", col = "Models") -> p_PAI
...
PAI_rf_ranked_grid  %>% ggplot() + geom_line(aes(x = prop_cum_area , y = prop_cum_crime_freq , col = "Random Forest")) +
  ...
  labs(title = "AUC of Models", subtitle = "Base Date Dec 31, 2019", col = "Models") -> p_AUC

( p_panel_stats = plot_grid(p_PAI, p_RRI, p_PEI, p_AUC, ncol = 2 ) )


panel_stats_plot_file = paste0(data_dir, "EXP_PANEL_STATS_PLOT_2019-12-31", ".png" )
...
perf_model_stats_file = paste0(data_dir, "EXP_MODEL_PERFORMANCE_STATS_2019-12-31.csv")
